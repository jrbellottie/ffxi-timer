...
### Printing
NPC resale planner covering every bundled synthesis recipe, including Shihei, Tsurara, Mahogany Pole, Ebony Pole, and Darksteel Pick. Enter all eight crafting skills and effective skill bonuses to rank eligible recipes by whole-chain gil/hour or profit per final synthesis attempt. Missing prices remain unknown, never free. Desynthesis is excluded; WotG, key-item recipes, and modern guild-rank stock are opt-in at every stage.

Material prices and ingredient source choices are shared across recipes and saved locally: choose Elm Log and enter 4000 gil each, for example. Recorded NPC offers seed raw-input acquisition prices when enabled; source lists link to NPC profiles. Auto expands every ingredient with a synthesis recipe in the current era/key-item filters, even when its buy price is known, its raw prices are missing, or its skill requirements are unmet. Among alternative crafting recipes it prefers the lowest fully priced crafting cost; unpriced chains are provisional. It does not optimize production speed. Only explicit Buy / gathered stops a craftable branch at a purchased item. Specific recipe choices remain fixed; unavailable stages retain their ingredient breakdown but block profit estimates. Output and ingredient recipe links retain Printing state on Back.

Ingredient production is recursive: Shihei automatically expands bast parchment into elm logs, moko grass, distilled water and a lightning crystal. Distilled water is deliberately a terminal purchased ingredient in Printing: its Tahrongi Cactus recipe is excluded, and its NPC price or your buy override is used instead. The general Crafting database is unchanged. Ink likewise expands through its chosen recipe. Parchment produces 12 on every successful roll, so HQ does not increase its yield. Each stage lists exact ingredients per synthesis and an NQ batch-allocation table. A 4000-gil elm log contributes `4000 / 12 * 2 = 666.67` gil to the two parchments used by one Shihei synthesis before other ingredients and break losses. Expected costs additionally account for losses and all earlier HQ yields. Mahogany lumber HQs produce 2/3/4 instead of 1, reducing the effective lumber cost and potentially creating the final pole's profit. Each stage shows its own success, skill requirements, HQ tier, expected usable yield and a four-tier unit-cost comparison. The final-item HQ comparison holds ingredient-stage skills fixed.

An ingredient's effective cost is expected consumed input cost per synthesis divided by expected usable output per synthesis. Only matching item IDs count toward usable yield; other outcomes receive no resale credit. Raw-input quantities and crystals are aggregated across the entire chain, and maximum purchase prices work backward from the final expected NPC revenue with other prices and source choices fixed. All intermediate material losses and extra crafting attempts are included. These are long-run averages, not exact finite-batch schedules or starting-capital estimates: surplus, inventory constraints, and shortfalls are not simulated. Circular branches or chains beyond the 32-stage safety limit remain unresolved with an explicit warning, never a silent purchase. Choose Buy at a boundary to provide a real acquisition price. The Printing tests audit every bundled synthesis recipe for complete ingredient quantities and unexplained unexpanded craftable branches.

Override each NQ/HQ item's sell value for actual server quotes or nerfs; zero means no resale proceeds. Clear an override to restore the bundled default. The headline compares buying all final ingredients with the selected production chain.

The model follows the checked LSB synthesis implementation at `3e73b0bd38626df86481547133332f0c1e59dc1d`: skill-surplus breakpoints 0/11/31/51, conditional HQ rates 1.5625/6.25/25/50%, lowest required craft limiting HQ, and a separate success roll for every required craft. Tier 0 produces HQ1 only; tier 1 HQs split 75/25/0%; tiers 2 and 3 split 75/18.75/6.25% across HQ1/HQ2/HQ3. These are configurable estimates, not verified live-server rates. Comparison rows assume all required skills reach each threshold, including potentially unattainable thresholds.

Break material loss defaults conservatively to 100%; checked LSB behavior is 50% at cap, rising below cap. The adjustable loss percentage is a scenario assumption applied at every stage, not an automatic below-cap loss simulation; no-loss recipes retain materials and crystals are always consumed. Bundled NPC BaseSell values are not final fame-adjusted quotes, and NoSale-flagged items default to zero proceeds. Explicit overrides can reflect a different server. NPC buy prices and availability depend on fame, conquest, guild stock and rank. Gear, food, day and moon modifiers are not inferred. The global sell multiplier also affects explicit sell overrides.
