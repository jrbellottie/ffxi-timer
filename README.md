...
### Profits
Profits has **Calculator** and **Prices** subtabs. Prices is the shared local price book for Profits and Chocobo Digging, the app's editable item-price calculators. It lists all crafting inputs (including crystals), NQ/HQ outputs, digging items, greens and weather items; All Items includes the wider bundled catalog. Search by item name, alias or ID, filter by crafting/digging, saved prices, missing market prices or explicit quotes, and browse 50 rows at a time. Prices view does not mount or recalculate the recipe list.

**Market / AH** is compared with the **Buy quote** to use the lower purchase price, or with the enabled NPC buying default when no buy quote is entered. Automatic selling uses the higher market price or NPC sale default. Original quotes are preserved; Applied shows the value used in calculations. NPC reference prices remain read-only. Enter prices per item or per stack; stack entries convert to per-item values internally without rounding away fractions. Zero is a real price, not missing data. A missing market price uses the quote/NPC baseline, or remains unknown if neither exists. Figures exclude AH fees and selling time.

Best-price selection is implicit, with no sale-mode dropdown. Entering a sell quote in Prices or a Profits output row forces that amount (including zero); enter the vendor price here when deliberately vendoring despite a higher AH price. Clearing the quote restores automatic selling. Market edits do not overwrite a manually entered sell quote. Older saved sell quotes without a manual choice remain automatic baselines compared against the market. Profits respects its recorded-NPC-buy and modern-guild-stock settings when comparing acquisition prices.

Edits in the central list, Profits recipe price fields, or Digging's AH fields use the same `kupo.item-prices.v1` store and are visible across tabs and reloads. Digging's AH field edits the shared market price; Applied shows the selected revenue price. Digging compares automatic sales with its existing vendor references, and its greens cost uses the shared lowest buy price. Built-in NPC vendor-revenue comparisons stay separate. The Prices shortcut in Digging opens the central book. Existing digging AH prices and separate Profits buy/sell quotes are migrated once, preserving conflicts as distinct quote types; old storage remains as a backup and is not reimported after a reset. Prices are local to this browser/Electron profile, not a live AH feed or cloud sync. Storage failures show a warning. Run `npm run test:prices` for migration, canonical aliases, persistence and calculation checks.

Crafting profit planner for NPC or auction-house sales, covering bundled synthesis recipes including Shihei, Tsurara, Mahogany Pole, Ebony Pole, and Darksteel Pick. Final recipes default to the reachable HQ tier with the highest whole-chain gil/hour. Select a row in Final recipe HQ comparison to use a specific tier; the choice is saved per recipe and applies to rankings, revenue, profit, ceilings and batch totals. Auto restores best-gil/hour selection and reacts to price changes. Equal or unknown returns fall back to the lowest required tier. All required main-craft and subcraft thresholds must be at most 106; higher thresholds are omitted. Ingredient stages retain the at-cap, HQ tier 0 baseline. No crafting-level input is needed. Missing prices remain unknown, never free. Desynthesis is excluded; WotG, key-item recipes, and modern guild-rank stock are opt-in at every stage. The internal `printing` tab ID and storage keys remain unchanged to preserve saved settings and navigation.

Material prices and ingredient source choices are shared across recipes and saved locally: choose Elm Log and enter 4000 gil each, for example. Recorded NPC offers seed raw-input acquisition prices when enabled; source lists link to NPC profiles. Auto expands every ingredient with a synthesis recipe in the current era/key-item filters, even when its buy price is known or its raw prices are missing. Among alternative crafting recipes it prefers the lowest fully priced crafting cost; unpriced chains are provisional. It does not optimize production speed. Only explicit Buy / gathered stops a craftable branch at a purchased item. Specific recipe choices remain fixed; unavailable stages retain their ingredient breakdown but block profit estimates. Output and ingredient recipe links retain Profits state on Back.

Ingredient production is recursive: Shihei automatically expands bast parchment into elm logs, moko grass, distilled water and a lightning crystal. Distilled water is deliberately a terminal purchased ingredient in Profits: its Tahrongi Cactus recipe is excluded, and its NPC price or your buy override is used instead. The general Crafting database is unchanged. Ink likewise expands through its chosen recipe. Parchment produces 12 on every successful roll, so HQ does not increase its yield. Each stage lists exact ingredients per synthesis and an NQ batch-allocation table. A 4000-gil elm log contributes `4000 / 12 * 2 = 666.67` gil to the two parchments used by one Shihei synthesis before other ingredients and break losses. Expected costs additionally account for losses and all earlier HQ yields. Mahogany lumber HQs produce 2/3/4 instead of 1, reducing the effective lumber cost and potentially creating the final pole's profit. Each stage shows its own success, skill requirements, HQ tier, expected usable yield and a unit-cost comparison for tiers reachable within 106 effective skill. The final-item HQ comparison holds ingredient stages at the at-cap, HQ tier 0 baseline.

An ingredient's effective cost is expected consumed input cost per synthesis divided by expected usable output per synthesis. Only matching item IDs count toward usable yield; other outcomes receive no resale credit. Raw-input quantities and crystals are aggregated across the entire chain, and maximum purchase prices work backward from the final expected sale revenue with other prices and source choices fixed. All intermediate material losses and extra crafting attempts are included. These are long-run averages, not exact finite-batch schedules or starting-capital estimates: surplus, inventory constraints, and shortfalls are not simulated. Circular branches or chains beyond the 32-stage safety limit remain unresolved with an explicit warning, never a silent purchase. Choose Buy at a boundary to provide a real acquisition price. The Profits tests audit every bundled synthesis recipe for complete ingredient quantities and unexplained unexpanded craftable branches.

Override each NQ/HQ item's per-unit sell value for NPC quotes or auction-house prices; a manually entered zero means no sale proceeds. Clear an override to restore automatic selling at the higher market or bundled NPC price. AH fees, unsold listings and time to sell are not included. The headline compares buying all final ingredients with the selected production chain.

Price fields apply after a 200 ms typing pause, or immediately on Enter or leaving the field. Price edits reuse cached estimates for unrelated recipes and recalculate affected ingredient chains, including alternative crafting sources, before updating the rankings. The latest calculation snapshot survives tab switches, including restored settings and Back navigation; the inactive tab's UI still unmounts. The first opening after an app reload builds the cache. Changes to crafting assumptions, source choices, vendor availability or recipe scope rebuild the model.

The model follows the checked LSB synthesis implementation at `3e73b0bd38626df86481547133332f0c1e59dc1d`: skill-surplus breakpoints 0/11/31/51, conditional HQ rates 1.5625/6.25/25/50%, lowest required craft limiting HQ, and a separate success roll for every required craft. Tier 0 produces HQ1 only; tier 1 HQs split 75/25/0%; tiers 2 and 3 split 75/18.75/6.25% across HQ1/HQ2/HQ3. These are configurable estimates, not verified live-server rates. Comparison rows assume all required skills reach each threshold, subject to the 106 effective-skill limit.

Material loss on break defaults to 50%, matching at-cap LSB behavior without loss-reduction modifiers. In [`handleMaterialLoss`](https://github.com/LandSandBoat/server/blob/3e73b0bd38626df86481547133332f0c1e59dc1d/src/map/utils/synthutils.cpp), the percentage is `clamp(50 - globalReduction - elementalReduction - craftReduction + 5 * max(synthDifficulty, 0), 20, 100)`, using the craft that failed and rolling separately for each ingredient slot. At cap, 30% would require reductions totaling 20 percentage points; it is not the unmodified default. These are checked LSB rules, not independently verified Phoenix server rates.

Expected ingredient consumption per attempt is `successChance + (1 - successChance) * lossPct / 100`: at 95% success and 50% loss on break, this is 97.5%. The adjustable loss percentage is a scenario assumption applied at every stage, not an automatic below-cap loss simulation; no-loss recipes retain materials and crystals are always consumed. Legacy saved 100% values migrate to 50% once; other custom values remain unchanged, and 100% can still be explicitly selected afterward. Bundled NPC BaseSell values are not final fame-adjusted quotes, and NoSale-flagged items default to zero proceeds. Explicit overrides can reflect a different server. NPC buy prices and availability depend on fame, conquest, guild stock and rank. Gear, food, day and moon modifiers are not inferred. The global sell multiplier also affects explicit sell overrides.

Gil/hour defaults to one synthesis every 21 seconds (adjustable from 20 to 22), with optional extra time per synthesis. Time includes the final attempt plus every required ingredient-production attempt. Batch size means final synthesis attempts, with total expected synthesis count and time shown separately. Estimates do not guarantee a realized return. Run `npm run test:printing` for model, chain and data checks.
