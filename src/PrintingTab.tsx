...
import { ArrowLeft, Coins, Hammer, RotateCcw, Search, Settings2, TrendingUp } from "lucide-react";
import { DEFAULT_PRINT_SETTINGS, HQ_GAPS, PRINT_CRAFTS, printingChances, type PrintSettings } from "./utils/printing";
...
type State = { settings: PrintSettings; buy: PriceBook; sell: PriceBook; sources: SourceChoices; sort: "hour" | "profit"; vendors: boolean; modernGuilds: boolean; query: string; craft: string; filter: string; wotg: boolean; keyItems: boolean; selected: number | null; batch: number };
const initial: State = { settings: DEFAULT_PRINT_SETTINGS, buy: {}, sell: {}, sources: {}, sort: "hour", vendors: true, modernGuilds: false, query: "", craft: "", filter: "eligible", wotg: false, keyItems: false, selected: null, batch: 100 };
...
function PriceField({ name, book, baseline, onChange, kind }: { name: string; book: PriceBook; baseline: number | null; onChange: (name: string, value: number | null) => void; kind: "Buy" | "Sell" }) {
  const override = book[printItemKey(name)];
  return <div className="print-price-field"><input type="number" min="0" step="1" aria-label={`${kind} ${name} gil each`} value={override ?? ""} placeholder={baseline === null ? "Unknown" : String(baseline)} onChange={(event) => {
    if (!event.target.value) onChange(name, null);
    else if (Number.isFinite(event.target.valueAsNumber) && event.target.valueAsNumber >= 0) onChange(name, event.target.valueAsNumber);
  }} /><button type="button" className="print-icon" disabled={override === undefined} aria-label={`Reset ${kind.toLowerCase()} price for ${name}`} title="Restore recorded price" onClick={() => onChange(name, null)}><RotateCcw size={14} /></button></div>;
}
...
type SourceProps = { node: ChainNode; demand: number | null; parentQuantity: number; state: State; chain: ReturnType<typeof createPrintingChain>; onSource: (name: string, choice: SourceChoice) => void; onPrice: (name: string, value: number | null) => void };

function IngredientSource({ node, demand, parentQuantity, state, chain, onSource, onPrice }: SourceProps) {
  ...
  const tiers = chainTierCosts(node, state.settings, printItemKey);
  ...
  const sourceControl = <label>Source for {node.name}<select aria-label={`Source for ${node.name}`} disabled={terminalWater} value={choice} onChange={(event) => onSource(node.name, event.target.value === "auto" || event.target.value === "buy" ? event.target.value : Number(event.target.value))}>
    ...
    {recipes.map((recipe) => { const chances = printingChances(recipe, state.settings); const output = chain.expectedYield(recipe, node.name); return <option key={recipe.id} value={recipe.id}>{recipe.craft} {recipe.lvl} / #{recipe.id} / {recipe.ing.map((item) => `${item.n} x${item.q}`).join(", ")}{!chances.eligible || chances.success <= 0 || output <= 0 ? " / Skill or yield unavailable" : ` / ${units(output)} usable per synth`}</option>; })}
  </select></label>;
  return <div className={`print-ingredient${node.recipe ? " print-crafted-ingredient" : ""}`} data-item={node.name}>
    ...
    <details className="print-sourcing"><summary>Sourcing &amp; NPC offers <span className="print-muted">/ {terminalWater || choice === "buy" ? "Buy" : choice === "auto" ? "Auto-craft" : `Recipe #${choice}`}{offers.length > 0 ? ` / ${offers.length} offers` : ""}</span></summary>
      <div className="print-source-controls">{sourceControl}<label>Buy quote / gil each<PriceField name={node.name} kind="Buy" book={state.buy} baseline={printBuyPrice(node.name, {}, state.vendors, state.modernGuilds)} onChange={onPrice} /></label></div>
    ...
    {node.recipe ? <details className="print-stage" open>
      ...
      <div className="print-ingredient-children">{node.children.map((child, index) => <IngredientSource key={`${child.node.key}:${index}`} {...{ state, chain, onSource, onPrice }} node={child.node} parentQuantity={child.recipeQuantity} demand={demand !== null && node.yield > 0 ? demand * child.quantity / node.yield : null} />)}</div>
    </details> : <p className="print-muted">{terminalWater ? "Terminal purchased ingredient" : node.issue ? "Unresolved ingredient branch" : choice === "buy" ? "Explicit Buy / gathered: breakdown stopped" : "No synthesis recipe in current era / key-item filters"} / {state.buy[node.key] !== undefined ? "your buy quote" : node.price === null ? "price needed" : "recorded NPC minimum"}.</p>}
  </div>;
}
...
export default function PrintingTab() {
  const [state, setState] = useState<State>(() => {
    ...
    return { ...initial, ...saved, settings: { ...DEFAULT_PRINT_SETTINGS, ...saved.settings }, query: peekNavQuery("printing") ?? saved.query ?? "" };
  });
  const [limit, setLimit] = useState(80);
  useEffect(() => { saveJson("kupo.printing.v1", state); rememberTabState("printing", state); }, [state]);
  ...
  const priceChange = (kind: "buy" | "sell", name: string, value: number | null) => setState((previous) => {
    const book = { ...previous[kind] };
    if (value === null) delete book[printItemKey(name)]; else book[printItemKey(name)] = value;
    return { ...previous, [kind]: book, selected: selected?.recipe.id ?? previous.selected };
  });
  const query = normalizeItemName(useDeferredValue(state.query));
  const available = useMemo(() => PRINT_RECIPES.filter((recipe) => (state.wotg || recipe.era !== "WotG") && (state.keyItems || !recipe.ki)), [state.wotg, state.keyItems]);
  const chain = useMemo(() => createPrintingChain(available, state.settings,
    (name) => printBuyPrice(name, state.buy, state.vendors, state.modernGuilds),
    (name) => printSellPrice(name, state.sell), printItemKey, state.sources),
  [available, state.settings, state.buy, state.sell, state.vendors, state.modernGuilds, state.sources]);
  const matches = available.filter((recipe) => (!state.craft || state.craft === recipe.craft) &&
    (!query || normalizeItemName([recipe.res.n, ...recipe.hq.map((item) => item.n), ...recipe.ing.map((item) => item.n)].join(" ")).includes(query)));
  const candidates = matches.map((recipe) => ({ recipe, estimate: chain.estimate(recipe) }));
  const ranked = candidates.filter(({ estimate }) => state.filter === "all" || (estimate.eligible && estimate.success > 0 &&
    (state.filter === "eligible" || (state.filter === "profitable" ? estimate.profit !== null && estimate.profit > 0 : estimate.profit === null))));
  ranked.sort((first, second) => Number(second.estimate.eligible && second.estimate.success > 0) - Number(first.estimate.eligible && first.estimate.success > 0) ||
    ...
    (second.estimate.revenue ?? 0) - (first.estimate.revenue ?? 0) || first.recipe.res.n.localeCompare(second.recipe.res.n) || first.recipe.id - second.recipe.id);
  ...
  const comparisons = recipe ? HQ_GAPS.map((_, tier) => chain.estimate(recipe, tier)) : [];
  const image = recipe ? wiki.items[printItemKey(recipe.res.n)]?.image : undefined;

  return <section className="printing-tab">
    <header className="print-heading"><h2><Coins size={23} /> Printing</h2><span className="print-muted">NPC resale / per-attempt economics</span>{hasBackTab() && <button className="print-command" onClick={goBackTab}><ArrowLeft size={16} /> Back</button>}</header>
    <details className="print-settings" open><summary><Settings2 size={16} /> Crafting skills</summary>
      <div className="print-skills">{PRINT_CRAFTS.map((craft) => <NumberField key={craft} label={craft} value={settings.skills[craft] ?? 0} max={110} step={0.1} onChange={(value) => setting({ skills: { ...settings.skills, [craft]: value } })} />)}</div>
    </details>
    <details className="print-settings"><summary>Economy &amp; crafting assumptions</summary>
      ...
      <h3>Effective skill bonuses</h3><div className="print-skills">{PRINT_CRAFTS.map((craft) => <NumberField key={craft} label={`${craft} bonus`} value={settings.bonuses[craft] ?? 0} max={30} onChange={(value) => setting({ bonuses: { ...settings.bonuses, [craft]: value } })} />)}</div>
      ...
      <p className="print-muted">Break loss defaults to 100% conservatively; checked LSB behavior is 50% at cap, increasing below cap. Crystals are always consumed. NPC sale defaults are bundled BaseSell values, not actual server quotes; NoSale items default to zero proceeds. Fame, nerfs, stock, conquest and guild restrictions can change prices. Sell multiplier applies to all sale values, including overrides. Confirm NPC acceptance in game.</p>
    </details>
    <div className="print-summary"><span><strong className="print-profit">{profitable.length}</strong> profitable with ingredient sourcing</span><span><strong>{missing.length}</strong> eligible, prices needed</span><span><strong>{gil(3600 / (settings.seconds + settings.overhead))}</strong> total synths/hour</span></div>
    <div className="print-filters"><label><span><Search size={15} /> Search recipes</span><input placeholder="Output or ingredient" value={state.query} onChange={(event) => { update({ query: event.target.value, selected: null }); setLimit(80); }} /></label><label>Craft<select value={state.craft} onChange={(event) => update({ craft: event.target.value, selected: null })}><option value="">All crafts</option>{PRINT_CRAFTS.map((craft) => <option key={craft}>{craft}</option>)}</select></label><label>View<select value={state.filter} onChange={(event) => update({ filter: event.target.value, selected: null })}><option value="eligible">Eligible recipes</option><option value="profitable">Profitable only</option><option value="missing">Needs prices</option><option value="all">All recipes, including locked</option></select></label><label>Rank by<select value={state.sort} onChange={(event) => update({ sort: event.target.value as State["sort"] })}><option value="hour">Whole-chain gil/hour</option><option value="profit">Profit / final attempt</option></select></label></div>
    <div className="print-workspace"><aside className="print-browser" aria-label="Ranked recipes"><div className="print-list-title">{ranked.length} recipes / {state.sort === "hour" ? "whole-chain gil/hour" : "gil per final attempt"}</div><div className="print-recipe-list">{ranked.slice(0, limit).map(({ recipe, estimate }, index) => <button type="button" key={recipe.id} aria-pressed={selected?.recipe.id === recipe.id} onClick={() => update({ selected: recipe.id })}><span className="print-recipe-title"><strong>{recipe.res.n}</strong><span className={tone(estimate.profit)}>{gil(state.sort === "hour" ? estimate.gilPerHour : estimate.profit)}</span></span><span className="print-muted">{index + 1}. {recipe.craft} {recipe.lvl}{recipe.subs?.map((sub) => ` / ${sub.c} ${sub.l}`).join("")} / #{recipe.id}</span><span className="print-muted">{!estimate.eligible || estimate.success <= 0 ? "Skill or ingredient chain unavailable" : estimate.tier < 0 ? "Below cap / no HQ" : `Final HQ tier ${estimate.tier} / ${pct(estimate.hq)}`}{estimate.profit !== null && ` / ${gil(estimate.profit)} gil per final attempt`}{recipe.ki && " / Key item"}</span></button>)}</div>{ranked.length > limit && <button className="print-command" onClick={() => setLimit(limit + 80)}>Show more recipes</button>}{!ranked.length && <p className="print-muted">No recipes match these skills and filters.</p>}</aside>
      {recipe && estimate ? <article className="print-detail" key={recipe.id}>
        ...
        {(!estimate.eligible || estimate.success <= 0) && <p className="print-warning">Final recipe or ingredient chain unavailable at current skills and source choices. {estimate.issues.join(". ")} Values below are theoretical.</p>}
        <div className="print-metrics"><div><span>Expected profit / final attempt</span><strong className={tone(estimate.profit)}>{gil(estimate.profit)}{estimate.profit !== null && " gil"}</strong></div><div><span>Whole-chain gil / hour</span><strong className={tone(estimate.gilPerHour)}>{gil(estimate.gilPerHour)}</strong></div><div><span>Expected NPC revenue</span><strong>{gil(estimate.revenue)}</strong></div><div><span>Expected raw-input cost</span><strong>{gil(estimate.cost)}</strong></div></div>
        ...
        <section className="print-section"><h3>Raw-input prices &amp; whole-chain ceilings</h3><div className="print-table-wrap"><table className="print-raw-inputs"><thead><tr><th>Raw input</th><th>Expected qty / final attempt</th><th>Buy gil each</th><th>Max gil each</th></tr></thead><tbody>{estimate.rawCeilings.map((input) => <tr key={input.key}><td><ItemButton name={input.name} /></td><td>{units(input.quantity)}</td><td><PriceField name={input.name} kind="Buy" book={state.buy} baseline={printBuyPrice(input.name, {}, state.vendors, state.modernGuilds)} onChange={(name, value) => priceChange("buy", name, value)} /><small>{state.buy[input.key] !== undefined ? "Your price" : input.price !== null ? "Recorded NPC minimum" : "Unknown"}</small></td><td className={input.maximum !== null && input.maximum < 0 ? "print-loss" : ""}>{gil(input.maximum)}</td></tr>)}</tbody></table></div><p className="print-muted">Break-even prices for the selected chain, holding all other raw prices and recipe choices fixed. Shared materials and crystals are combined across every stage. Unknown prices are not zero; negative ceilings cannot break even with that input free.</p></section>
        <section className="print-section"><h3>Ingredient production chain</h3><div className="print-table-wrap"><table><thead><tr><th>Final ingredient</th><th>Recipe qty</th><th>Effective cost each</th><th>Max cost each</th></tr></thead><tbody>{estimate.ceilings.map((input) => <tr key={input.name}><td><ItemButton name={input.name} /></td><td>{input.quantity}</td><td>{gil(input.price)}</td><td>{gil(input.maximum)}</td></tr>)}</tbody></table></div>
          <p className="print-muted">Auto expands every craftable ingredient within the era / key-item filters, even with unknown prices or unmet skills. Alternative recipes use the lowest fully priced crafting cost when available; Buy explicitly stops a branch. HQ yields, break losses, crystals and batch quantities apply at every stage. Circular or more than {MAX_CHAIN_DEPTH}-stage branches are flagged, never silently purchased. Nonmatching outputs have no credit. Finite batches can leave surplus or fall short.</p>
          <div className="print-production">{estimate.sources.map((source, index) => <IngredientSource key={`${source.node.key}:${index}`} node={source.node} demand={source.quantity} parentQuantity={estimate.inputs[index].quantity} state={state} chain={chain} onSource={sourceChange} onPrice={(name, value) => priceChange("buy", name, value)} />)}</div>
        </section>
        <section className="print-section"><h3>NPC output values</h3><div className="print-table-wrap"><table><thead><tr><th>Roll</th><th>Output</th><th>Qty</th><th>Chance / attempt</th><th>Sell gil each</th></tr></thead><tbody>{estimate.outcomes.map((outcome, index) => <tr key={index}><td>{index ? `HQ${index}` : "NQ"}</td><td><ItemButton name={outcome.n} /><small>Base {gil(printSellPrice(outcome.n, {}))}</small></td><td>{outcome.q}</td><td>{pct(outcome.probability)}</td><td><PriceField name={outcome.n} kind="Sell" book={state.sell} baseline={printSellPrice(outcome.n, {})} onChange={(name, value) => priceChange("sell", name, value)} /><small>Applied {gil(outcome.price)} gil</small></td></tr>)}</tbody></table></div><p className="print-muted">Sale overrides apply to the same item across all recipes and HQ rows. Values of 0 have no NPC resale return.</p></section>
        <section className="print-section"><h3><TrendingUp size={16} /> Final recipe HQ comparison</h3><div className="print-table-wrap"><table><thead><tr><th>Tier</th><th>Required effective skills</th><th>HQ</th><th>Profit / final attempt</th><th>Chain gil / hour</th></tr></thead><tbody>{comparisons.map((comparison, index) => <tr key={index} className={estimate.tier === index ? "print-current-tier" : ""}><td>{index} (+{HQ_GAPS[index]})</td><td>{comparison.requirements.map((requirement) => `${requirement.craft} ${requirement.cap + HQ_GAPS[index]}`).join(", ")}</td><td>{pct(comparison.hq)}</td><td className={tone(comparison.profit)}>{gil(comparison.profit)}</td><td className={tone(comparison.gilPerHour)}>{gil(comparison.gilPerHour)}</td></tr>)}</tbody></table></div><p className="print-muted">Only the final recipe changes tier; ingredient stages retain entered skills. Some thresholds are unattainable on your server. Stage-specific HQ comparisons appear with each crafted ingredient.</p></section>
      ...
      </article> : <div className="print-empty"><Coins size={30} /><p>No matching printing candidates.</p></div>}
    </div>
  </section>;
}