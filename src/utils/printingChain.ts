...
export function createPrintingChain(recipes: PrintRecipe[], settings: PrintSettings, buy: (name: string) => number | null, sell: (name: string) => number | null, itemKey: (name: string) => string, choices: SourceChoices = {}) {
  ...
  const cache = new Map<string, ChainNode>();
  ...
  const estimate = (recipe: PrintRecipe, tier?: number) => {
    ...
    return { ...result, eligible: result.eligible && !issues.length, sources, leaves, rawCeilings, issues, totalAttempts, seconds,
  ...
  return { resolve, estimate, options, expectedYield };
}
